# c02-mq-reliable-delivery —— 第二话配套代码

对应文章：[第二话｜消息队列的可靠投递与可靠消费](https://zh.renxinblog.cn/post/gyd-c02-mq-reliable-delivery/)
...
| 端点（POST） | 做什么 | 正文 |
|---|---|---|
| `/gyd/c02/order?orderId=o-1` | 下一单并投递，同步等 Broker 回执；`orderId` 含 `fail` 走「失败→重试→死信」，含 `slow` 每条多处理 `gyd.c02.slow-processing-ms`（默认 200ms） | 一、三、五 |
| `/gyd/c02/order/unroutable?orderId=order-3001` | 投一条路由键无人绑定的消息，观察 return 与 confirm 同时出现 | 一 |
| `/gyd/c02/order/probe?id=p-1&persistent=true\|false` | 投一条探针消息到无消费者的队列 `gyd.c02.persist.probe`，重启 broker 前后对比数量 | 二 |
| `/gyd/c02/order/burst?prefix=slow&count=50` | 批量投递灌出稳定积压，采样 prefetch 与未确认条数 | 三 |

## 拓扑

| 名称 | 值 |
|---|---|
| 业务交换机 / 队列 / 路由键 | `gyd.c02.order.exchange` / `gyd.c02.order.queue` / `order.created` |
| 重试交换机 / 队列 / 路由键 | `gyd.c02.order.retry.exchange` / `gyd.c02.order.retry.queue` / `order.retry` |
| 重试延迟（队列级 TTL） | `RETRY_TTL_MS = 2000` |
| 最终死信交换机 / 队列 / 路由键 | `gyd.c02.order.dlx` / `gyd.c02.order.dlq` / `order.dead` |
| 持久化探针队列 | `gyd.c02.persist.probe`（不绑交换机、不挂消费者） |

链路：业务队列 --nack(requeue=false)--> 重试交换机 → 重试队列 --TTL 到期--> 业务交换机 → 业务队列；
重试用尽时应用显式投到 `gyd.c02.order.dlx`，再 ack 掉原消息（不能 nack，否则再次进入重试循环）。
...
```
$ curl -X POST 'localhost:8080/gyd/c02/order/unroutable?orderId=order-3001'

[producer] 已投递到无人绑定的路由键: orderId=order-3001
[return] 消息未能路由到队列: exchange=gyd.c02.order.exchange, routingKey=order.nowhere, replyText=NO_ROUTE
[confirm] Broker 已确认消息: id=order-3001
```

查 `gyd.c02.order.queue` 与 `gyd.c02.order.retry.queue` 深度均为 0。
...
[consumer] 处理失败，2000ms 后重试（第 2/3 次）: x-death=gyd.c02.order.retry.queue/expired ×1 + gyd.c02.order.queue/rejected ×1
[consumer] 处理失败，2000ms 后重试（第 3/3 次）: x-death=gyd.c02.order.retry.queue/expired ×2 + gyd.c02.order.queue/rejected ×2
[consumer] 重试 3 次仍失败，转入死信队列: orderId=order-fail-retry-2, 累计投递 4 次
```

相邻两次处理的间隔实测 2004 / 2004 / 2005ms（重跑有几毫秒浮动），与队列上配的 2000ms TTL 对得上。
