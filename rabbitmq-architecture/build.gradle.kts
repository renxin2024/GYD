// RabbitMQ 架构与特性（GYD 系列前置篇）。本篇**刻意使用裸客户端 amqp-client，
...
// 用它反而看不到本篇要展示的东西（与 c03 选裸客户端同源）。
//
// 版本基线：amqp-client 5.30.0 —— 与 c01（spring-boot-starter-amqp，由 Boot 4.1.1 的 BOM
// 选定的版本）**同一个客户端版本**（已用 `:c01-mq-reliable-delivery:dependencyInsight
...
application {
    // 用法：./gradlew :rabbitmq-architecture:run --args="<demo>"
