# c01-rabbitmq-architecture —— 第一话配套代码

对应文章：[第一话｜RabbitMQ 的架构与特性：一条消息的一生](https://zh.renxinblog.cn/post/gyd-c01-rabbitmq-architecture/)
...
./gradlew :c01-rabbitmq-architecture:run --args="<示例名>"
```
...
- 所有 AMQP 资源统一用 `gyd.ra.` 前缀，与 c02 的 `gyd.c02.*`、c03 的 `gyd.c03.*` 隔离，互不干扰。
...
```bash
./gradlew :c01-rabbitmq-architecture:run --args="persistence-write"
docker restart gyd-rabbitmq
./gradlew :c01-rabbitmq-architecture:run --args="persistence-read"
```
