...
GYD 第 2 篇[《跨行清算对账》](https://zh.renxinblog.cn/post/gyd-c02-reconciliation/)的配套代码。
...
|------|------|---------|------|--------|------|
| 结算服务 | `settlement` | 无 | 18080 | `gyd_c02_settlement` | 扮演央行：产出权威结算记录 + 跑对账 Job |
| 工行 | `bank` | `bank-a` | 18081 | `gyd_c02_bank_a` | 付款行（A 的开户行） |
| 建行 | `bank` | `bank-b` | 18082 | `gyd_c02_bank_b` | 收款行（B 的开户行） |
...
```bash
./gradlew c02-reconciliation:settlement:bootJar c02-reconciliation:bank:bootJar
```
...
**IDEA 里模块名和文件夹名对不上。** 不是配置错了，是 IDEA 导入 Gradle 工程的固定命名规则：模块名 = `<rootProject.name>` + `.` + 各级工程名（路径段用点连接），并且**每个 source set 还会单独生成一个模块**加 `.main` / `.test` 后缀。于是文件夹 `c02-reconciliation/bank` 对应的模块叫 `gyd.c02-reconciliation.bank`，另外还多出 `…bank.main`、`…bank.test` 两个磁盘上并不存在的模块。前缀 `gyd.` 是固定的，去不掉。能优化的是这几处：

- 子模块目录不带 `c02-` 前缀——外层已经是 `c02-reconciliation/`，再带一次会让模块名里 `c02` 出现两次（`gyd.c02-reconciliation.c02-bank`）。
