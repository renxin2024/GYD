package cn.renxinblog.gyd.c02.shared.ledger;

import cn.renxinblog.gyd.c02.shared.domain.Account;
...
public class JournalEntry {
    ...
    /** 科目标识，取值见 {@link Account#code()}：customer_deposit（客户存款）/ pboc_settlement（存放央行款项） */
    ...
    /** 是否为冲正分录（由冲正生成、指向某条原始分录） */
    public boolean isReversal() {
        return reversalOf != null;
    }

    /**
     * 可读的一行分录，用于日志和文章分录表对照。
     * 例：{@code 借 客户存款 ¥100.00 (tradeId=TRD-1, entryId=E-TRD-1-ICBC-CUST)}
     */
    public String describe() {
        return String.format("%s %s ¥%.2f (tradeId=%s, entryId=%s%s)",
                direction.label(), accountId, amountCents / 100.0, tradeId, entryId,
                isReversal() ? ", 冲正 " + reversalOf : "");
    }
}