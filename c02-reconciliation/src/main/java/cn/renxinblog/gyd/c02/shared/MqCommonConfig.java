 ...
 *
 * 这里有两个职责，都是「无 @Profile」的共享 bean，三个服务（结算、付款行、收款行）
 * 都要加载：
 *
 ...
 *    反序列化 {@link cn.renxinblog.gyd.c02.settlement.SettlementRecord}，不能只在
 *    某一个 profile 下装配，否则另一端会退回默认的 SimpleMessageConverter
 *    （Java 序列化），导致反序列化失败。
 *
